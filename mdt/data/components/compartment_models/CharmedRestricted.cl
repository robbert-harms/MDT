...
model_float cmCharmedRestricted(const double4 g,
    ...
    const float gamma_cyl_radii_sq[] = {2.25e-12, 6.25e-12, 1.225e-11,
                                         2.025e-11, 3.025e-11, 4.224999e-11};
    const float gamma_cyl_weights[] = {0.0211847200855742, 0.107169623942214, 0.194400551313197,
                                        0.266676876170322, 0.214921653661151, 0.195646574827541};

    // tmp0 is also used in the loop, do not reassign it easily.
    double tmp0 = pown(dot(g, (double4)(cos(phi) * sin(theta),
                                        sin(phi) * sin(theta), cos(theta), 0.0)), 2);
    const double tmp1 = (-d * b * tmp0);
    const double tmp2 = ((-GAMMA2_G2_delta2 * (1 - tmp0)) / (d * (TE / 2.0))) * (7/96.0);
