...
def get_charmed(nmr_restr=3):
    if nmr_restr == 3:
        ...
        name = 'CharmedBall_' + repr(nmr_restr) + 'r'

    description = 'Extended charmed model with an extra isotropic compartment, a Ball. ' \
                  'Contains {0} restricted compartments.'.format(nmr_restr)
    ...
    isotr = (compartments_loader.get_constructor('Weight')('w_iso'),
             compartments_loader.get_constructor('Ball')('Ball'),
             '*')

    ml = (compartments_loader.load('S0'), (hin, res, isotr, '+'), '*')
    ...
    def model_construction_cb(evaluation_model=SumOfSquares(), signal_noise_model=JohnsonSignalNoise()):
        ...
        modifiers = [('FR', lambda results: 1 - results['w_hin0.w']),
                     ('SNIF', lambda results: 1 - results['w_iso.w'])]
