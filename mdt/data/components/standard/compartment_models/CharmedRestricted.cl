...
MOT_FLOAT_TYPE cmCharmedRestricted(const MOT_FLOAT_TYPE4 g,
    ...
    const MOT_FLOAT_TYPE gamma_cyl_weights[] = {
    ...
    const MOT_FLOAT_TYPE dotted = pown(dot(g, (MOT_FLOAT_TYPE4)(cos(phi) * sin(theta),
                                        sin(phi) * sin(theta), cos(theta), 0.0)), 2);
    const MOT_FLOAT_TYPE tmp1 = (-d * b * dotted);
    const MOT_FLOAT_TYPE tmp2 = ((-GAMMA2_G2_delta2 * (1 - dotted)) / (d * (TE / 2.0))) * (7/96.0);
